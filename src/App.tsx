...
const CVS_NUTRITION_DB: Record<string, { pre: string[], post: string[], rationale: string }> = {
  'quadriceps_and_glutes': {
    ...
    rationale: '最高碳水需求，支援下肢大肌群高強度功率輸出。'
  },
  'back_and_biceps': {
    pre: ['鮪魚飯糰 + 黑咖啡 (提升神經專注力)', '燕麥飲 + 香蕉 (高鉀電解質)'],
    post: ['鮪魚沙拉 + 鮪魚飯糰 + 優格 (P ≈ 35g)', '鹽味雞胸肉 + 巧克力牛奶 (P ≈ 30g)'],
    rationale: '中碳水 + 高蛋白，維持拉類動作爆發力與背部修復。'
  },
  'chest_and_triceps': {
    pre: ['鮭魚飯糰 + 豆漿 (均衡供能)', '總匯三明治 + 黑咖啡 (快速排空)'],
    post: ['雞胸肉 + 高蛋白牛乳 (P ≈ 40g)', '溫泉蛋兩顆 + 雞肉沙拉 + 豆漿 (P ≈ 35g)'],
    rationale: '極高蛋白質攝取，優化推類動作後強烈的肌肉肥大反應。'
  },
  'hamstrings_and_calves': {
    pre: ['地瓜 + 香蕉 (高纖與速效碳水)', '燕麥飲 + 豆漿 (流質好吸收)'],
    post: ['雞胸肉 + 地瓜 + 牛奶 (P ≈ 32g)', '鮪魚沙拉 + 烤地瓜 + 優格飲 (P ≈ 28g)'],
    rationale: '中高碳水，補足後側動力鏈高離心訓練的代謝壓力。'
  },
  'shoulders_and_core': {
    pre: ['香蕉 + 黑咖啡 (輕量供能)', '無加糖輕量優格 (極速胃排空)'],
    post: ['豆腐沙拉 + 茶葉蛋 + 豆漿 (P ≈ 27g)', '毛豆 + 雞胸肉片 + 無糖綠茶 (P ≈ 25g)'],
    rationale: '低碳水 + 高蛋白，控制熱量避免無謂脂肪囤積。'
  }
};
...
export default function App() {
  ...
  // ✨ 核心改造 1：狀態改為「陣列 (Array)」，支援多選
  const [targetMuscleGroups, setTargetMuscleGroups] = useState<string[]>(['quadriceps_and_glutes']);
  ...
  const [result, setResult] = useState<{
    ...
    cvs_protocol: { pre: string, post: string, rationale: string }; 
  } | null>(null);
  ...
  useEffect(() => {
    ...
    if (month >= 3 && month <= 5) {
      setCurrentSeason('春季');
      setSeasonalFruits('聖女小番茄、草莓或枇杷 (富含維生素C與茄紅素)');
    } else if (month >= 6 && month <= 8) {
      setCurrentSeason('夏季');
      setSeasonalFruits('愛文芒果、西瓜或火龍果 (光速補醣與一氧化氮生成)');
    } else if (month >= 9 && month <= 11) {
      setCurrentSeason('秋季');
      setSeasonalFruits('麻豆文旦、柿子或水梨 (抗氧化劑與穩定膳食纖維)');
    } else {
      setCurrentSeason('冬季');
      setSeasonalFruits('柳丁、蜜棗或橘子 (協助合成第一型膠原蛋白)');
    }
  }, []);
  ...
  const generatePrescription = async () => {
    ...
    try {
      if (availableEquipment.length === 0) throw new Error('請至少選擇一項可用器材。');
      // ✨ 阻擋無選擇肌群防呆
      if (targetMuscleGroups.length === 0) throw new Error('請至少選擇一個訓練目標肌群。');
      ...
      // ✨ 核心改造 3：代謝壓力覆蓋機制 (最高層級優先)
      let metabolicDemand = 'low';
      if (targetMuscleGroups.some(g => ['quadriceps_and_glutes', 'back_and_biceps'].includes(g))) {
        metabolicDemand = 'high';
      } else if (targetMuscleGroups.some(g => g === 'chest_and_triceps')) {
        metabolicDemand = 'moderate';
      }

      // ✨ 核心改造 4：多重肌群 CVS 輪替抽樣池 (Data Pooling)
      let combinedPre: string[] = [];
      let combinedPost: string[] = [];
      let combinedRationales: string[] = [];

      targetMuscleGroups.forEach(group => {
        const cvsData = CVS_NUTRITION_DB[group];
        combinedPre.push(...cvsData.pre);
        combinedPost.push(...cvsData.post);
        // 抓出中文名稱用於解釋
        const labelCn = MUSCLE_GROUPS.find(m => m.id === group)?.label.match(/\((.*?)\)/)?.[1] || group;
        combinedRationales.push(`【${labelCn}】${cvsData.rationale}`);
      });

      const randomPre = combinedPre[Math.floor(Math.random() * combinedPre.length)];
      const randomPost = combinedPost[Math.floor(Math.random() * combinedPost.length)];
      const cvsRationaleText = combinedRationales.join(' ');
      ...
      if (isInflamed) {
        ...
        if (metabolicDemand === 'high') {
           nutrition += `本次複合菜單包含大肌群，誘發較高發炎反應，建議額外補充高劑量 Omega-3，並搭配台灣${currentSeason}盛產的 ${seasonalFruits}，強效清除活性氧類 (ROS)。`;
        } else {
           nutrition += `局部代謝壓力相對較低。建議以台灣${currentSeason}當季的 ${seasonalFruits} 為主，協助清除游離自由基。`;
        }
      } else if (isFatigued) {
        const sourceText = dataSource === 'wearable' ? '心率變異度 (HRV) 低落或睡眠嚴重不足' : '細胞相位角 (PhA < 5.0) 偏低';
        nutrition = `⚡ [中樞神經修復介入] 偵測到 ${sourceText}，反映顯著的神經損耗。`;
        if (metabolicDemand === 'high') {
           nutrition += `黃金窗口內必須強制補充高生物價蛋白質，並攝取台灣${currentSeason}特產的 ${seasonalFruits} 加速 ATP-PC 系統回補。`;
        } else {
        ...
        if (metabolicDemand === 'high') {
           nutrition += `🎯 因應複合肌群產生的高耗能，啟動「高碳水/高蛋白」策略，餐後補充台灣${currentSeason}盛產的 ${seasonalFruits}，以微量營養素矩陣最大化 mTORC1 肌肉合成訊號。`;
        } else {
      ...
      // ✨ 核心改造 5：複合部位的力學迴圈 (Biomechanical Loop)
      targetMuscleGroups.forEach(group => {
        switch (group) {
          case 'quadriceps_and_glutes':
            if (femurRatio === 'high') {
               reasoning.push('針對「長股骨」，優化深蹲力學減少腰椎剪力，重點轉移至下肢推舉。');
               ...
               reasoning.push('針對正常/短股骨，建議執行常規深蹲動作。');
          ...
          case 'back_and_biceps':
            reasoning.push('背二頭著重於垂直與水平拉力動作的平衡。');
          ...
          case 'hamstrings_and_calves':
             if (!isInflamed) {
                ...
                reasoning.push('因發炎指標偏高，排除 RDL 等高離心動作。');
             }
      ...
      setResult({
        recommended_exercises: [...new Set(exercises)], // 利用 Set 自動濾除重複的動作
        biomechanical_reasoning: [...new Set(reasoning)].join(' '), // 濾除重複的推導語句
        autoregulation_feedback: feedback.join(' '),
        nutrition_prescription: nutrition,
        cvs_protocol: { pre: randomPre, post: randomPost, rationale: cvsRationaleText } 
      });
        ...
        <header className="mb-8 flex items-center justify-between print:hidden">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-400 font-mono mt-1">v4.3 // Multi-Muscle Composite Engine</p>
            </div>
          </div>
        </header>
          ...
          <div className={`lg:col-span-5 space-y-6 print:hidden ${isExporting ? 'hidden' : ''}`}>
            ...
            <div className="bg-slate-900/50 backdrop-blur-sm border border-slate-800 rounded-2xl p-6">
              ...
              <div className="flex bg-[#1a1a1a] rounded-xl p-1 border border-gray-800 mb-6">
                <button onClick={() => setDataSource('wearable')} className={`flex-1 flex items-center justify-center gap-2 py-2.5 text-sm font-medium rounded-lg transition-all ${dataSource === 'wearable' ? 'bg-blue-600 text-white shadow-md' : 'text-gray-400 hover:text-gray-200'}`}>
                  <Watch className="w-4 h-4" />日常穿戴快篩
                </button>
                <button onClick={() => setDataSource('inbody')} className={`flex-1 flex items-center justify-center gap-2 py-2.5 text-sm font-medium rounded-lg transition-all ${dataSource === 'inbody' ? 'bg-purple-600 text-white shadow-md' : 'text-gray-400 hover:text-gray-200'}`}>
                  <Microscope className="w-4 h-4" />月度 InBody
                </button>
              </div>
            ...
            {/* ✨ UI 改造：支援多選且有視覺回饋的目標肌群 */}
            <div className="bg-slate-900/50 backdrop-blur-sm border border-slate-800 rounded-2xl p-6">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  <Target className="w-5 h-5 text-blue-400" />
                  <h2 className="text-lg font-semibold text-gray-200">Target Muscle Groups <span className="text-xs font-normal text-gray-500 ml-2">(支援多選)</span></h2>
                </div>
              </div>
              <div className="flex flex-wrap gap-2">
                {MUSCLE_GROUPS.map(group => {
                  const isSelected = targetMuscleGroups.includes(group.id);
                  return (
                    <button 
                      key={group.id} 
            ...
            <button onClick={generatePrescription} disabled={loading} className="w-full py-4 bg-blue-600 hover:bg-blue-500 disabled:bg-blue-600/50 text-white rounded-xl font-bold text-lg transition-all flex items-center justify-center gap-3 shadow-[0_0_20px_rgba(37,99,235,0.2)]">
              {loading ? <><Loader2 className="w-6 h-6 animate-spin" /><span>運算複合處方中...</span></> : <><Rocket className="w-6 h-6" /><span>🚀 生成綜合生物力學處方</span></>}
            </button>
            ...
            <div className="bg-slate-900/50 backdrop-blur-sm border border-slate-800 rounded-2xl p-6 h-full min-h-[600px] flex flex-col">
              <div className="flex items-center justify-between mb-6 pb-4 border-b border-gray-800">
                <div className="flex items-center gap-2">
                  ...
                  <h2 className="text-lg font-semibold text-gray-200">Composite Results</h2>
                </div>
                {result && (
              ...
              {result ? (
                <div className="space-y-8 flex-1 overflow-y-auto pr-2">
                  <section>
                    <h3 className="text-sm font-mono mb-3 uppercase tracking-wider text-gray-500">Composite Exercises (綜合菜單)</h3>
                  ...
                  <section>
                    <h3 className="text-sm font-mono mb-3 uppercase tracking-wider text-purple-400/80">🏪 Multi-Muscle CVS Protocol (複合採購指南)</h3>
                    <div className="p-5 bg-purple-950/20 border border-purple-900/30 rounded-lg relative overflow-hidden space-y-4">
                      ...
                      <div className="flex flex-col gap-2 border-b border-purple-900/50 pb-3">
                        ...
                        <div className="flex items-center gap-2 text-sm text-gray-300">
                           ...
                           <span>{result.cvs_protocol.pre}</span>
                        </div>
                      </div>
                      ...
                      <div className="flex flex-col gap-2 border-b border-purple-900/50 pb-3">
                        ...
                        <div className="flex items-center gap-2 text-sm text-gray-300">
                           <ShoppingBag className="w-4 h-4 text-gray-500 shrink-0" />
                           <span>{result.cvs_protocol.post}</span>
                        </div>
                      </div>

                      <div>
                        <span className="text-xs font-bold text-gray-500">綜合生理機制：</span>
                ...
                <div className="flex-1 flex flex-col items-center justify-center text-gray-500 space-y-4">
                  ...
                  <p className="text-sm font-mono text-center max-w-xs">等待輸入參數... 支援部位複選的決策系統已就緒。</p>
                </div>
              )}
            </div>
          </div>
      ...
      <div className="overflow-hidden h-0 w-0 absolute top-[-9999px] left-[-9999px]">
        <div ref={exportRef} style={{ width: '800px', backgroundColor: '#ffffff', padding: '40px', color: '#0f172a' }}>
          <div style={{ marginBottom: '32px', borderBottom: '2px solid #1e293b', paddingBottom: '16px' }}>
            <h1 style={{ fontSize: '24px', fontWeight: 'bold', color: '#0f172a', margin: 0 }}>Antigravity Fitness OS // 綜合生物力學與營養處方</h1>
          ...
          {result && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
              <section>
                <h3 style={{ fontSize: '14px', fontFamily: 'monospace', marginBottom: '12px', textTransform: 'uppercase', letterSpacing: '0.05em', color: '#64748b' }}>Composite Exercises</h3>
              ...
              <section>
                <h3 style={{ fontSize: '14px', fontFamily: 'monospace', marginBottom: '12px', textTransform: 'uppercase', letterSpacing: '0.05em', color: '#7e22ce' }}>CVS Nutrition Protocol</h3>
                <div style={{ padding: '20px', backgroundColor: '#f3e8ff', border: '1px solid #d8b4fe', borderRadius: '8px', position: 'relative', overflow: 'hidden' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                    <div>
                      <span style={{ fontSize: '12px', fontWeight: 'bold', color: '#9333ea' }}>PRE-WORKOUT</span>
                    ...
                    <div style={{ borderTop: '1px solid #e9d5ff', paddingTop: '12px' }}>
                      <span style={{ fontSize: '12px', fontWeight: 'bold', color: '#9333ea' }}>POST-WORKOUT</span>
